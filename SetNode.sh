...
#	Version: 2.0.38 (Fixed by Assistant)
#	Author: Toyo (Fixed version)
...
BBR_installation_status(){
	if [[ ! -e ${BBR_file} ]]; then
		...
		cd "${file}"
		# 修复：使用替代 BBR 脚本地址
		if ! wget -N --no-check-certificate https://raw.githubusercontent.com/chiakge/Linux-NetSpeed/master/tcp.sh -O bbr.sh; then
...
Get_IP(){
	...
	if [[ -z "${ip}" ]]; then
		...
		if [[ -z "${ip}" ]]; then
			# 修复：移除失效的 3322.org 地址
			ip=$(wget -qO- -t1 -T2 api.ipify.org)
			if [[ -z "${ip}" ]]; then
				ip="VPS_IP"
			fi
		fi
...
View_User(){
	...
	if [[ -z "${now_mode}" ]]; then
		...
		user_total=`${jq_file} '.port_password' ${config_user_file} |sed '$d;1d' | wc -l`
		user_total=`expr ${user_total} + 1`
		for((integer = 1; integer <= ${user_total}; integer++))
		do
			port=`${jq_file} '.port_password' ${config_user_file} |sed '$d;1d' |awk -F ":" '{print $1}' |sed -n "${integer}p" |sed -r 's/.*\"(.+)\".*/\1/'`
			password=`${jq_file} '.port_password' ${config_user_file} |sed '$d;1d' |awk -F ":" '{print $2}' |sed -n "${integer}p" |sed -r 's/.*\"(.+)\".*/\1/'`
			ss_ssr_determine
			clear && echo "===================================================" && echo
			echo -e " ShadowsocksR账号 配置信息 [${Green_font_prefix}${integer}${Font_color_suffix}/${Green_font_prefix}${user_total}${Font_color_suffix}]：" && echo
			echo -e " I  P\t    : ${Green_font_prefix}${ip}${Font_color_suffix}"
			echo -e " 端口\t    : ${Green_font_prefix}${port}${Font_color_suffix}"
			echo -e " 密码\t    : ${Green_font_prefix}${password}${Font_color_suffix}"
			echo -e " 加密\t    : ${Green_font_prefix}${method}${Font_color_suffix}"
...
Set_config_obfs(){
 ...
 ${Tip} 如果使用 ShadowsocksR 加速游戏，建议选择 混淆兼容原版或 plain 混淆，然后客户端选择 plain，否则会增加延迟 !" && echo
...
Set_config_protocol_param(){
	...
	echo -e "${Tip} 设备数限制：每个端口同一时间能链接的IP数量，最少可填 2，最大可填 65535(理论上更大，但没必要)，建议填 3 或 5 即可。"
	read -e -p "(默认: 无限):" ssr_protocol_param
	[[ -z "$ssr_protocol_param" ]] && ssr_protocol_param="" && echo && return
	if [[ $ssr_protocol_param -ge 2 ]] && [[ $ssr_protocol_param -le 65535 ]]; then
		echo && echo ${Separator_1} && echo -e "	设备数限制 : ${Green_font_prefix}${ssr_protocol_param}${Font_color_suffix}" && echo ${Separator_1} && echo
	else
		echo -e "${Error} 请输入正确的数字(2-65535)" && exit 1
	fi
}
Set_config_speed_limit_per_con(){
	...
	if [[ $? == 0 ]]; then
		if [[ ${ssr_speed_limit_per_con} -ge 1 ]] && [[ ${ssr_speed_limit_per_con} -le 99999999 ]]; then
			...
			echo -e "${Error} 请输入正确的数字(1-99999999)"
		fi
	else
		echo -e "${Error} 请输入正确的数字(1-99999999)"
	fi
...
Set_config_speed_limit_per_user(){
	...
	if [[ $? == 0 ]]; then
		if [[ ${ssr_speed_limit_per_user} -ge 1 ]] && [[ ${ssr_speed_limit_per_user} -le 99999999 ]]; then
...
Set_config_transfer(){
	while true
	do
	echo -e "请输入要设置的ShadowsocksR账号 可使用的总流量上限(单位: GB, 1-838868 GB)"
	...
	if [[ $? == 0 ]]; then
		if [[ ${ssr_transfer} -ge 1 ]] && [[ ${ssr_transfer} -le 838868 ]]; then
			echo && echo ${Separator_1} && echo -e "	总流量 : ${Green_font_prefix}${ssr_transfer} GB${Font_color_suffix}" && echo ${Separator_1} && echo
			break
		else
...
Set_config_enable(){
	...
	do
		port=`${jq_file} '.port_password' ${config_user_file} |sed '$d;1d' |awk -F ":" '{print $1}' |sed -n "${integer}p" |sed -r 's/.*\"(.+)\".*/\1/'`
		echo && echo -e "请输入该端口 [${Green_font_prefix}${port}${Font_color_suffix}] 的用户名" && read -e -p "(默认: 省略):" user_name
		[[ -z "${user_name}" ]] && user_name=""
		user_name_all=${user_name_all}"## "${user_name}"\n"
	done
	echo -e "${user_name_all}" > "${config_folder}/userapiconfig.py"
	user_name_all=""
	echo -e "${Info} 写入完成！"
}
Set_config_all(){
...
Set_config_server(){
	echo && echo -e "请输入要配置的服务器公网IP"
	read -e -p "(自动检测IPv4地址，IPv6请手动输入):" ssr_server
	if [[ -z "${ssr_server}" ]]; then
		Get_IP
		ssr_server="${ip}"
	fi
	echo && echo ${Separator_1} && echo -e "	IP : ${Green_font_prefix}${ssr_server}${Font_color_suffix}" && echo ${Separator_1} && echo
}
# 修改 配置信息
Modify_config_port(){
	Set_config_port
	Modify_json="s/\"server_port\": \"[0-9]\+\"/\"server_port\": \"${ssr_port}\"/g"
	sed -i "${Modify_json}" ${config_user_file}
	echo -e "${Info} ShadowsocksR账号 端口修改成功！"
}
Modify_config_password(){
	Set_config_password
	Modify_json="s/\"password\": \".*\"/\"password\": \"${ssr_password}\"/g"
	sed -i "${Modify_json}" ${config_user_file}
	echo -e "${Info} ShadowsocksR账号 密码修改成功！"
}
Modify_config_method(){
	Set_config_method
	Modify_json="s/\"method\": \".*\"/\"method\": \"${ssr_method}\"/g"
	sed -i "${Modify_json}" ${config_user_file}
	echo -e "${Info} ShadowsocksR账号 加密方式修改成功！"
}
Modify_config_protocol(){
	Set_config_protocol
	Modify_json="s/\"protocol\": \".*\"/\"protocol\": \"${ssr_protocol}\"/g"
	sed -i "${Modify_json}" ${config_user_file}
	echo -e "${Info} ShadowsocksR账号 协议修改成功！"
}
Modify_config_obfs(){
	Set_config_obfs
	Modify_json="s/\"obfs\": \".*\"/\"obfs\": \"${ssr_obfs}\"/g"
	sed -i "${Modify_json}" ${config_user_file}
	echo -e "${Info} ShadowsocksR账号 混淆修改成功！"
}
Modify_config_protocol_param(){
	Set_config_protocol_param
	Modify_json="s/\"protocol_param\": \".*\"/\"protocol_param\": \"${ssr_protocol_param}\"/g"
	sed -i "${Modify_json}" ${config_user_file}
	echo -e "${Info} ShadowsocksR账号 设备数限制修改成功！"
}
Modify_config_speed_limit_per_con(){
	Set_config_speed_limit_per_con
	Modify_json="s/\"speed_limit_per_con\": [0-9]*/\"speed_limit_per_con\": ${ssr_speed_limit_per_con}/g"
	sed -i "${Modify_json}" ${config_user_file}
	echo -e "${Info} ShadowsocksR账号 单线程限速修改成功！"
}
Modify_config_speed_limit_per_user(){
	Set_config_speed_limit_per_user
	Modify_json="s/\"speed_limit_per_user\": [0-9]*/\"speed_limit_per_user\": ${ssr_speed_limit_per_user}/g"
	sed -i "${Modify_json}" ${config_user_file}
	echo -e "${Info} ShadowsocksR账号 用户总限速修改成功！"
}
Modify_config_connect_verbose_info(){
	Modify_json="s/\"connect_verbose_info\": [0-9]*/\"connect_verbose_info\": ${ssr_connect_verbose_info}/g"
	sed -i "${Modify_json}" ${config_user_file}
}
Modify_config_all(){
	Modify_config_port
...
Modify_config_server_pub_addr(){
	Modify_json="s/\"server\": \".*\"/\"server\": \"${ssr_server}\"/g"
	sed -i "${Modify_json}" ${config_user_file}
}
# 安装 依赖
...
Update_SSR(){
	SSR_installation_status
	echo -e "因原版仓库已删除，更新功能已被禁用。建议手动备份配置后重新安装。"
	echo -e "${Tip} 请使用选项 3 卸载后重新安装以获取最新版本。"
}
Uninstall_SSR(){
...
Check_Libsodium_ver(){
	...
	# 修复：使用 GitHub API 获取最新版本更可靠
	Libsodiumr_ver=$(wget -qO- "https://api.github.com/repos/jedisct1/libsodium/releases/latest" | grep -o '"tag_name": ".*"' | sed 's/"tag_name": "//;s/"//g')
...
Download_SSR(){
	cd "/usr/local/"
	# 修复：使用可用的备份仓库
	wget -N --no-check-certificate "https://github.com/shadowsocksrr/shadowsocksr/archive/3.2.2.tar.gz" -O shadowsocksr.tar.gz
	[[ ! -e "shadowsocksr.tar.gz" ]] && echo -e "${Error} ShadowsocksR服务端 压缩包 下载失败 !" && rm -rf shadowsocksr.tar.gz && exit 1
	tar -xzf shadowsocksr.tar.gz
	# 修复：修改解压后的目录名
	[[ ! -e "/usr/local/shadowsocksr-3.2.2/" ]] && echo -e "${Error} ShadowsocksR服务端 解压失败 !" && rm -rf shadowsocksr.tar.gz && exit 1
	mv "/usr/local/shadowsocksr-3.2.2/" "/usr/local/shadowsocksr/"
	[[ ! -e "/usr/local/shadowsocksr/" ]] && echo -e "${Error} ShadowsocksR服务端 重命名失败 !" && rm -rf shadowsocksr.tar.gz && rm -rf "/usr/local/shadowsocksr-3.2.2/" && exit 1
	rm -rf shadowsocksr.tar.gz
...
# description: ShadowsocksR start/stop/restart
...
do_start(){
	...
	if [[ $? -eq 0 ]]; then
		...
		python_ver="python"
		ulimit -n 512000
		nohup "${python_ver}" "$BIN" a >> /dev/null 2>&1 &
		sleep 2s
		check_running
		if [[ $? -eq 0 ]]; then
...
case "$1" in
	...
	;;
	*)
	echo "使用方法: service ssr {start|stop|restart|status}"
	RETVAL=1
	;;
esac
exit $RETVAL
EOF
	...
	echo -e "${Info} ShadowsocksR服务 管理脚本创建完成 !"
}
# 安装 JQ解析器
JQ_install(){
	if [[ ! -e ${jq_file} ]]; then
		...
		# 修复：更新 jq 下载地址到最新版本
		if [[ ${bit} = "x86_64" ]]; then
			wget --no-check-certificate "https://github.com/jqlang/jq/releases/download/jq-1.7.1/jq-linux-amd64" -O ${jq_file}
		else
			wget --no-check-certificate "https://github.com/jqlang/jq/releases/download/jq-1.7.1/jq-linux-arm64" -O ${jq_file}
		fi
...
get_IP_address(){
	if [[ ! -z ${user_IP_1} ]]; then
		...
		do
			...
			IP_address=`wget -qO- -t1 -T2 http://freeapi.ipip.net/${IP}|sed 's/\"//g;s/\[//g;s/\]//g'`
...
View_Config(){
	...
	Get_User
	echo && echo "===================================================" && echo
	echo -e " 你的ShadowsocksR配置信息：" && echo
	echo -e " I  P\t    : ${Green_font_prefix}${ip}${Font_color_suffix}"
	echo -e " 端口\t    : ${Green_font_prefix}${port}${Font_color_suffix}"
	echo -e " 密码\t    : ${Green_font_prefix}${password}${Font_color_suffix}"
	echo -e " 加密\t    : ${Green_font_prefix}${method}${Font_color_suffix}"
...
Modify_Config(){
 ...
 ${Green_font_prefix}4.${Font_color_suffix}  修改 全局配置
————————————————
 ${Green_font_prefix}0.${Font_color_suffix}  返回 主菜单" && echo
...
Port_mode_switching(){
	SSR_installation_status
	[[ ! -e ${config_user_file} ]] && echo -e "${Error} 没有发现ShadowsocksR配置文件 !" && exit 1
	now_mode=$(cat "${config_user_file}"|grep '"port_password"')
	if [[ -z "${now_mode}" ]]; then
		echo && echo -e "	当前模式: ${Green_font_prefix}单端口${Font_color_suffix}" && echo
		echo -e "确定要切换为 ${Green_font_prefix}多端口${Font_color_suffix} 模式？[y/N]"
		read -e -p "(默认: n):" mode_yn
		[[ -z ${mode_yn} ]] && mode_yn="n"
		if [[ ${mode_yn} == [Yy] ]]; then
			user_total=`${jq_file} '.port_password' ${config_user_file} |sed '$d;1d' | wc -l`
			[[ "$user_total" != "0" ]] && echo -e "${Error} 当前已有多端口配置，请先清空 !" && exit 1
			port=`${jq_file} '.server_port' ${config_user_file}`
			password=`${jq_file} '.password' ${config_user_file} | sed 's/^.//;s/.$//'`
			#sa=`${jq_file} '.additional_ports' ${config_user_file}`
			#echo -e "${sa}"
			Set_config_all
			Write_configuration_manyuser
			Del_iptables
		...
		echo -e "确定要切换为 ${Green_font_prefix}单端口${Font_color_suffix} 模式？[y/N]"
		read -e -p "(默认: n):" mode_yn
		[[ -z ${mode_yn} ]] && mode_yn="n"
		if [[ ${mode_yn} == [Yy] ]]; then
			user_total=`${jq_file} '.port_password' ${config_user_file} |sed '$d;1d' | wc -l`
...
View_Log(){
	SSR_installation_status
	[[ ! -e ${ssr_log_file} ]] && echo -e "${Error} 没有找到日志文件 !" && exit 1
...
# 修复：锐速服务已失效，提示用户并建议使用 BBR
Install_ServerSpeeder(){
	echo -e "${Error} 锐速(Server Speeder) 安装脚本已失效！"
	echo -e "${Tip} 建议使用 BBR 加速替代，请在主菜单选择 14 -> 2 安装 BBR。"
	exit 1
}
Uninstall_ServerSpeeder(){
...
# 修复：LotServer 服务已失效，提示用户并建议使用 BBR
Install_LotServer(){
	echo -e "${Error} LotServer 安装脚本已失效！"
	echo -e "${Tip} 建议使用 BBR 加速替代，请在主菜单选择 14 -> 2 安装 BBR。"
	exit 1
}
Uninstall_LotServer(){
	...
	[[ -z ${unyn} ]] && echo && echo "已取消..." && exit 1
	if [[ ${unyn} == [Yy] ]]; then
		echo -e "${Error} 卸载脚本已失效，请手动删除 /appex 目录。"
	fi
}
# BBR
Configure_BBR(){
...
Other_functions(){
  ...
  ${Green_font_prefix}1.${Font_color_suffix} 配置 BBR 加速
  ${Green_font_prefix}2.${Font_color_suffix} 配置 Sharp Speed 加速(锐速)
  ${Green_font_prefix}3.${Font_color_suffix} 配置 LotServer 加速
————————————
  ${Green_font_prefix}4.${Font_color_suffix} 封禁 BT PT SPAM
  ${Green_font_prefix}5.${Font_color_suffix} 解封 BT PT SPAM
  ${Green_font_prefix}6.${Font_color_suffix} 切换 ShadowsocksR 日志输出模式
  " && echo
...
# 封禁 BT PT SPAM - 修复：使用内嵌规则替代失效脚本
BanBTPTSPAM(){
	echo -e "${Info} 开始封禁 BT/PT/SPAM 端口..."
	# BT 常用端口
	iptables -A INPUT -m multiport -p tcp --dports 6881:6889,6969,51413 -j DROP
	iptables -A INPUT -m multiport -p udp --dports 6881:6889,6969,51413 -j DROP
	# PT 常用端口
	iptables -A INPUT -p tcp --dport 9091 -j DROP
	# SPAM 常用端口
	iptables -A INPUT -p tcp --dport 25 -j DROP
	echo -e "${Info} BT/PT/SPAM 端口封禁完成！"
	echo -e "${Tip} 这些规则将在重启后失效，如需永久保存请执行：iptables-save"
}
# 解封 BT PT SPAM
UnBanBTPTSPAM(){
	echo -e "${Info} 开始解封 BT/PT/SPAM 端口..."
	# BT 常用端口
	iptables -D INPUT -m multiport -p tcp --dports 6881:6889,6969,51413 -j DROP 2>/dev/null
	iptables -D INPUT -m multiport -p udp --dports 6881:6889,6969,51413 -j DROP 2>/dev/null
	# PT 常用端口
	iptables -D INPUT -p tcp --dport 9091 -j DROP 2>/dev/null
	# SPAM 常用端口
	iptables -D INPUT -p tcp --dport 25 -j DROP 2>/dev/null
	echo -e "${Info} BT/PT/SPAM 端口解封完成！"
}
Set_config_connect_verbose_info(){
	SSR_installation_status
	Get_User
...
# 修复：禁用脚本自动更新功能，因为原仓库已删除
Update_Shell(){
	echo -e "${Error} 脚本自动更新功能已禁用！"
	echo -e "${Info} 原仓库 ToyoDAdoubi/doubi 已被删除。"
	echo -e "${Tip} 请手动下载最新版本的修复脚本。"
	exit 0
}
# 显示 菜单状态
menu_status(){
...
echo -e "  ShadowsocksR 一键管理脚本 ${Red_font_prefix}[v${sh_ver}]${Font_color_suffix} (Fixed)
